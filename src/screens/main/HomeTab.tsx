    ...
    <section className="h-full relative flex flex-col">
      <div className="flex-1 overflow-y-auto pb-[140px]">
        <div className="px-8 pt-6">
          ...
          {homeStep < 2 && (
            <p
              className={`text-center font-black tracking-[-0.03em] text-[#6E31E8] ${
                homeStep === 0 ? "mt-5 text-[22px]" : "mt-16 text-[40px]"
              }`}
            >
            ...
            {homeStep === 0
              ? currentQuestion?.options.map((item: any) => (
                  <button
                    ...
                    className={`h-[140px] rounded-[40px] bg-white shadow ${
                      mood === item.label
                  ...
                  >
                    ...
                    <div className="mt-2 font-black text-2xl">{item.label}</div>
                  </button>
                ))
              : currentQuestion?.options.map((item: any) => (
                  <button
                    key={item.label}
                    onClick={() => setPlace(item.label)}
                    className={`h-[140px] rounded-[40px] bg-white shadow flex flex-col items-center justify-center ${
                      place === item.label
                        ? "border-[6px] border-purple-500 scale-105"
                        : ""
                    }`}
                  >
                    <div className="text-5xl">{item.emoji}</div>
                    <div className="mt-3 text-2xl font-black text-[#6E31E8]">
                      {item.label}
                    </div>
                  </button>
                ))}
